...
const Companies = () => {
  ...
  const handleResetPassword = async (company) => {
    const newPassword = window.prompt(
      `Enter new temporary password for ${company.name} admin:`,
    );
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/auth/admin-reset-password/${company.id}`,
        {
          method: "PUT",
          headers: {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-2">
              Manage transit cooperatives and their details.
            </p>
          </div>
          <button
            onClick={handleAddNewClick}
            className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg flex items-center gap-2"
          >
                    ...
                    >
                      ...
                      <td className="p-4">
                        ...
                        <div className="text-emerald-700 font-bold text-sm mt-1 bg-emerald-50 inline-block px-2 py-0.5 rounded">
                          <span className="text-emerald-600/70 text-xs mr-1">
                            LOGIN:
                          </span>
                      ...
                      <td className="p-4 flex justify-end gap-2">
                        <button
                          ...
                          className="p-2 text-amber-600 bg-amber-50 hover:bg-amber-100 rounded-lg"
    ...
    <div className="max-w-4xl mx-auto animate-fade-in-up">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {status.message && (
            <div
              className={`p-4 rounded-xl flex items-center gap-3 font-medium ${status.type === "success" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-red-50 text-red-600 border border-red-100"}`}
            >
              {status.type === "success" ? (
          ...
          <div className="pt-4 flex justify-end">
            <button
              type="submit"
              disabled={isLoading}
              className="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg transform hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-70 flex items-center gap-2"
            >
              {isLoading ? (
