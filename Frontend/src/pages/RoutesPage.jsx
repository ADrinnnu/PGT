...
const RoutesPage = () => {
  ...
  const calculateRoadRoute = async (start, end) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.code === "Ok") {
        const roadCoordinates = data.routes[0].geometry.coordinates.map(
  ...
  const uniqueCompanies = [
    "All",
    ...new Set(routes.map((r) => r.companyName).filter(Boolean)),
  ];
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-2">
            Map routes, calculate distances, and set custom pricing.
          </p>
        </div>
        <button
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-8 animate-fade-in-up">
          ...
          <div className="mb-8">
            <div className="flex justify-between items-end mb-3">
              <p className="text-sm text-slate-500 font-medium">
                Click Origin, then click Destination to snap to roads.
              </p>
              <button
                type="button"
                onClick={clearMap}
                disabled={waypoints.length === 0}
                className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-lg disabled:opacity-50 transition-colors"
              >
            ...
            <div className="h-96 w-full rounded-xl overflow-hidden border-2 border-slate-200 shadow-inner z-0 relative">
              {isCalculating && (
                <div className="absolute inset-0 bg-white/60 z-[9999] flex items-center justify-center font-bold text-emerald-700 backdrop-blur-sm">
                  Calculating Route & Distance...
                </div>
              )}
              <MapContainer
                center={[15.48, 120.597]}
                zoom={11}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    readOnly
                    value={`${formData.distanceKm} km`}
                    className="w-full pl-10 pr-4 py-3 bg-blue-50 border border-blue-200 text-blue-800 font-bold rounded-xl outline-none"
                  />
                </div>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full pl-10 pr-4 py-3 bg-emerald-50 border border-emerald-200 text-emerald-800 font-bold rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none"
                  />
                </div>
              </div>
            </div>
            {status.message && (
              <div
                className={`p-4 rounded-xl flex items-center gap-3 font-medium ${status.type === "success" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}
              >
                {status.message}
              </div>
            )}
            <div className="flex justify-end">
              <button
                type="submit"
                disabled={isSubmitting || routePath.length === 0}
                ...
                >
                  <td className="p-4 font-bold text-slate-800 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-blue-500 font-medium mt-0.5 uppercase tracking-tighter">
                        Click to view map
                      </div>
                    </div>
                  </td>
                  ...
                  {isHeadAdmin && (
                    <td className="p-4 text-emerald-700 font-bold bg-emerald-50/50 uppercase tracking-tight">
                      {route.companyName}
                    </td>
                  )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-slate-500 font-medium mt-1">
                  {viewRoute.origin} to {viewRoute.destination} •{" "}
                  {viewRoute.distanceKm} km
                </p>
              </div>
              <button
            ...
            <div className="h-[500px] w-full bg-slate-100 relative z-0">
              {getSavedCoordinates(viewRoute.routeCoordinates).length > 0 ? (
                <MapContainer
                  center={getSavedCoordinates(viewRoute.routeCoordinates)[0]}
                  zoom={12}
                  className="h-full w-full"
                >
                  <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                  <Marker
                    position={
                      getSavedCoordinates(viewRoute.routeCoordinates)[0]
                    }
                    icon={customIcon}
                  />
                  <Marker
                    position={
                      getSavedCoordinates(viewRoute.routeCoordinates)[
                        getSavedCoordinates(viewRoute.routeCoordinates).length -
                          1
                      ]
                    }
                    icon={customIcon}
                  />
                  <Polyline
                    positions={getSavedCoordinates(viewRoute.routeCoordinates)}
                    color="#059669"
                    weight={6}
                    opacity={0.8}
                  />
                ...
                <div className="flex h-full items-center justify-center text-slate-400 font-medium">
                  No GPS data was saved for this older route.
                </div>
              )}
            </div>
          </div>
